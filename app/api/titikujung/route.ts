...
export async function GET(request: Request) {
  try {
    ...
    const filePath = path.join(process.cwd(), 'public', 'data', 'titikujung_7hari.geojson');
    ...
    return NextResponse.json({
      ...
      metadata: {
        total_titik: filteredFeatures.length,
    ...
    return NextResponse.json(
      { error: 'Gagal memuat titik ujung GeoJSON', details: error.message },
      { status: 500 }
    );
  }
}
